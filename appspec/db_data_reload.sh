...
then
  ...
  dbsynced=$(mysql -u "$dbuser" -p"$dbpass" -h "$dbhost" -e "SELECT count(*) as '' FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '$dbname' AND TABLE_NAME = 'createdflag'")
  if [ dbsynced = "0" ]
  then
    ...
    rm db.sql
    mysql -u "$dbuser" -p"$dbpass" -h "$dbhost" -e "use $dbname; CREATE TABLE createdflag(flag int);"
  fi
fi
cd /var/app/current
